...
export const getUniqueClasses = (subject: string): string[] => {
  const chapters = mockChapters.filter((ch) => ch.subject === subject);
  const classes = new Set<string>();
  chapters.forEach((ch) => classes.add(ch.class));
  return Array.from(classes).sort();
};

export const getUniqueUnits = (subject: string): string[] => {
  const chapters = mockChapters.filter((ch) => ch.subject === subject);
  const units = new Set<string>();
  chapters.forEach((ch) => units.add(ch.unit));
